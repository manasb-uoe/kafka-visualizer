...
export class ApiService {
  ...
  public getTopicDataUnVersioned(
    topic: KafkaTopic,
    partition: number,
    query: string
  ): Observable<Array<TopicMessage>> {
    ...
    return this.http
      .get(`/api/topics/${topic.name}/${partition}?query=${query}&`)
      .map(response => response.json().data);
  }

  public getTopicConsumers(
    topic: KafkaTopic,
    partition: number
  ): Observable<Array<Consumer>> {
    return this.http
      .get(`/api/consumers/${topic.name}/${partition}`)
      .map(response => response.json());
  }

  public publishTopicMessage(
    topic: KafkaTopic,
    key: string,
    value: string
    ...
    return this.http
      .post(`/api/topics/${topic.name}`, body.toString())
      .map((response: any) => {
        response = response.json();
        return new RecordMetadata(
          response.offset,
          response.topicPartition.partition
        );
      });
  }

  private getVersionedResponseStream<T>(
    endpoint: string,
    period: number
  ): Observable<T> {
  ...
  private getResponseStream<T>(
    ...
    const handler = () => {
      this.http
        .get(isVersioned ? this.createVersionedEndpoint(endpoint, version) : endpoint)
        .map(response => response.json())
        .subscribe(
          response => {
            if (isVersioned) {
